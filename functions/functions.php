...
/// begin pcc_add functions ///

function pcc_add($selected_termekek){
        global $db;
        $ip_add = getRealIpUser();
    
        $product_qty = 1;
    
        foreach ($selected_termekek as $p_id) {
            $check_product = "select * from cart where ip_add='$ip_add' AND p_id='$p_id'";
                
            $run_check = mysqli_query($db,$check_product);       
                    
            if(mysqli_num_rows($run_check)>0){
                echo "<script>alert('Egy vagy több kiválasztott termék már a kosárban van, így nem kerül(nek) az(ok) be a kosárba!')</script>";
            }else{
                $query = "insert into cart (p_id,ip_add,qty) values ('$p_id','$ip_add','$product_qty')";
                ...
                if(!$run_query){
                    echo "<script>alert('Hiba a kosárba tételnél')</script>";
                }
            }
        }
}
